...
app.post("/api/puppies", async (req: Request, res: Response) => {
  try {
    ...
    await fs.writeFile(
      "data/puppies.json",
      JSON.stringify(puppiesArray, null, 2)
    );
    return res
      .status(200)
      .json(puppiesArray.find(puppy => puppy.id == newPuppie.id));
  } catch (err) {
    console.log(err);
    return res.status(401).json({ message: err.message });
  }
...
app.delete("/api/puppies/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const index = puppiesArray.puppies.findIndex(
      puppy => Number(id) === puppy.id
    );
